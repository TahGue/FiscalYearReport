  ...
  return (
    <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="flex items-center justify-between gap-3">
        <div>
          <h3 className="text-base font-semibold text-slate-800">Sparande möjligheter</h3>
          <p className="mt-1 text-sm text-slate-500">Högpåverkande åtgärder baserat på ditt verkliga utgiftsbeteende.</p>
        </div>
        <div className="rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-right">
          <p className="text-xs text-emerald-700">Potentiellt månadsvis</p>
          <p className="text-lg font-semibold text-emerald-800">{fmt(totalMonthly, currency)}</p>
        </div>
      </div>

      {opportunities.length === 0 ? (
        <p className="mt-3 text-sm text-slate-500">Inte tillräckligt med diskretionära/återkommande signaler ännu för att rekommendera sparandeåtgärder.</p>
      ) : (
        <ul className="mt-3 space-y-2">
          {opportunities.map((opportunity) => (
            <li key={opportunity.id} className="rounded-lg border border-slate-200 p-3 text-sm">
              ...
              <p className="mt-2 text-xs text-slate-700">Åtgärd: {opportunity.action}</p>
              <div className="mt-2 flex gap-3 text-xs">
                <span className="rounded bg-emerald-50 px-2 py-1 text-emerald-700">Månadsvis {fmt(opportunity.monthlySaving, currency)}</span>
                <span className="rounded bg-slate-100 px-2 py-1 text-slate-700">Årligen {fmt(opportunity.annualSaving, currency)}</span>
              </div>
            </li>
          ))}
        </ul>
