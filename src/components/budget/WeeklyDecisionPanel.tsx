...
export default function WeeklyDecisionPanel({ txs, budgetBands, bufferTargets, currency }: Props) {
  ...
  return (
    <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
      <h3 className="text-base font-semibold text-slate-800">Weekly summary & next best actions</h3>
            ...
            <li key={action.id} className="rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm">
              <div className="flex items-center justify-between gap-3">
                ...
                <span className="rounded bg-emerald-100 px-2 py-0.5 text-xs font-medium text-emerald-700">
                  {action.expectedMonthlyImpact > 0 ? `+${fmt(action.expectedMonthlyImpact, currency)}/mo` : "stability action"}
                </span>
              </div>
              <p className="mt-1 text-xs text-slate-600">Why this appears: {action.explainability}</p>
            </li>
          ))}
        </ul>
      )}
